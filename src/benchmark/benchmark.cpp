...
#ifdef OPENCL
#include <libubqhash-cl/ubqhash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(ubqhash_h256_t const *hash, unsigned size)
{
...
{
	// params for ubqhash
	ubqhash_params params;
	ubqhash_params_init(&params, 0);
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		ubqhash_return_value hash;
		ubqhash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("ubqhash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		ubqhash_return_value hash;
		ubqhash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : ubqhash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			ubqhash_return_value hash;
			ubqhash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			ubqhash_return_value hash;
			#ifdef FULL
				ubqhash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				ubqhash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
